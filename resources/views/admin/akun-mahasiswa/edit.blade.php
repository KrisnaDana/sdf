    ...
    <div class="col-md-12">
        ...
        <div class="row">
                ...
                <div class="col">
                    ...
                    <a href="{{route('admin-view-edit-akun-mahasiswa', ['id' => $mahasiswa->id])}}"><button type="button" class="btn cur-p btn-lg btn-primary mr-3" style="float: right;"><i class="fa fa-refresh"></i></button></a>
                </div>
            </div>
        </div>
    </div>
...
<div class="row">
    <div class="col">
        <div class="white_shd full margin_bottom_30">
            <form method="post" action="{{route('admin-edit-akun-mahasiswa', ['id' => $mahasiswa->id])}}">
                @csrf
                <div class="table_section padding_infor_info">
                    <div class="mb-3">
                        <label class="form-label">NIM <span style="color:#FF0000">*</span></label>
                        <input type="number" class="form-control @error('nim') is-invalid @enderror" name="nim" value="{{old('nim') ? old('nim') : $mahasiswa->nim}}" spellcheck="disabled" required>
                        @error('nim')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('nama_lengkap') is-invalid @enderror" name="nama_lengkap" value="{{old('nama_lengkap') ? old('nama_lengkap') : $mahasiswa->nama_lengkap}}" spellcheck="disabled" required>
                        @error('nama_lengkap')
                        <div class="invalid-feedback">{{$message}}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        @else
                            ...
                            @foreach($jalur_pendaftarans as $jalur_pendaftaran)
                                @if(old('jalur_pendaftaran_id') == $jalur_pendaftaran->id || (empty(old('jalur_pendaftaran_id')) && $mahasiswa->jalur_pendaftaran_id == $jalur_pendaftaran->id))
                    ...
                    <div class="mb-3">
                        ...
                        @else
                            ...
                            @foreach($program_studis as $program_studi)
                                @if(old('program_studi_id') == $program_studi->id || (empty(old('program_studi_id')) && $mahasiswa->program_studi_id == $program_studi->id))
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('angkatan') is-invalid @enderror" name="angkatan" spellcheck="disabled" value="{{ old('angkatan') ? old('angkatan') : $mahasiswa->angkatan}}" required>
                        @error('angkatan')
                        <div class="invalid-feedback">{{$message}}</div>
                        @enderror
                    </div>
