            ...
            <div class="row">
                ...
                <div class="col">
                    <a href="{{route('admin-view-create-gugus')}}"><button type="button"
                            class="btn cur-p btn-lg btn-success mr-3" style="float: right;">Tambah</button></a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col">
        <div class="white_shd full margin_bottom_30 padding_40">
            <div class="padding_infor_info" style="display:flex;">
                ...
                <div class="col">
                    <a href="{{route('admin-view-gugus')}}"><button type="button" class="btn btn-outline-secondary m-0"
                            id="btn-refresh" style="float:right;"><i
                                class="fa fa-refresh text-secondary"></i></button></a>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <table class="table" id="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{$loop->index+1}}</td>
                                <td>{{$gugus->gugus}}</td>
                                <td>{{$gugus->link_gugus ? $gugus->link_gugus : "-"}}</td>
                                ...
                                <td class="text-center">
                                    <a href="{{route('admin-read-gugus', ['id' => $gugus->id])}}"><button type="button"
                                            class="btn btn-primary"><i class="fa fa-book text-white"></i></button></a>
                                    <a href="{{route('admin-view-edit-gugus', ['id' => $gugus->id])}}"><button
                                            type="button" class="btn btn-warning"><i
                                                class="fa fa-pencil-square text-white"></i></button></a>
                                    <button type="button" class="btn btn-danger" data-toggle="modal"
                                        data-target="#delete-modal{{$loop->index+1}}"><i
                                            class="fa fa-trash-o text-white"></i></button>
                                </td>
                            </tr>
                            <div class="modal fade" id="delete-modal{{$loop->index+1}}">
                                <form method="post" action="{{route('admin-delete-gugus', ['id' => $gugus->id])}}">
                                    @csrf
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                Apakah anda yakin ingin menghapus {{$gugus->nama}}?
                                            </div>
