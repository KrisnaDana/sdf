...
<div class="row column_title">
    <div class="col-md-12">
        ...
        <div class="row">
                ...
                <div class="col">
                    <a href="{{route('admin-view-create-periode-pendaftaran')}}"><button type="button" class="btn cur-p btn-lg btn-success mr-3" style="float: right;">Tambah</button></a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col">
        <div class="white_shd full margin_bottom_30 padding_40">
            <div class="padding_infor_info" style="display:flex;">
                ...
                <div class="col">
                    <a href="{{route('admin-view-periode-pendaftaran')}}"><button type="button" class="btn btn-outline-secondary m-0" id="btn-refresh" style="float:right;"><i class="fa fa-refresh text-secondary"></i></button></a>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="table-responsive-sm">
                    <table class="table" id="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{$loop->index+1}}</td>
                                <td>{{$periode_pendaftaran->program_studi->nama}}</td>
                                <td>{{$periode_pendaftaran->jalur_pendaftaran->nama}}</td>
                                <td>{{$periode_pendaftaran->mulai}}</td>
                                <td>{{$periode_pendaftaran->berakhir}}</td>
                                <td class="text-center">
                                    <a href="{{route('admin-view-edit-periode-pendaftaran', ['id' => $periode_pendaftaran->id])}}"><button type="button" class="btn btn-warning"><i class="fa fa-pencil-square text-white"></i></button></a>
                            ...
                            <div class="modal fade" id="delete-modal{{$loop->index+1}}">
                                <form method="post" action="{{route('admin-delete-periode-pendaftaran', ['id' => $periode_pendaftaran->id])}}">
                                    @csrf
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                Apakah anda yakin ingin menghapus periode tersebut?
                                            </div>
